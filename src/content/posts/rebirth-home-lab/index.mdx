...
- Disable access time on service datasets
- Use `smartmontools` to monitor disks SMART status
- Use `btop` to monitor system resources
...
### Setup periodic ZFS snapshots and retention policy
...
There are apparently ways to simulate memory errors, I'll have to look into it.
...
- Use GitHub webhooks to trigger actions on the main server, such as building and deploying the hosted websites.