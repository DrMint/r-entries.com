...
If everything is working, let's now disable password authentication.
...
Fail2Ban is a tool that bans IPs that try to brute force SSH.
In our case, we already disabled password authentication, so its usefulness is limited.
Still, it's a low effort setup that can help protect against malformed requests and other hammering attacks.
...
It will ban an IP for 1 hour after 5 failed attempts within a 10-minute window.
...
You can check if any IP is banned with:

```sh
fail2ban-client status sshd
```

If you are willing to be locked out of your server for 1 hours, you can try to wrongly connect 5 times to see if fail2ban works.
This time, keeping a SSH connection open to test Fail2Ban will not work because it's your IP that is banned, which is the same across your terminals.
Alternatively, if you stuck yourself and you can't wait, you can restart the server using the power button. Bans are not persisted across restarts.
...
- Same OS installation procedure
- Same SSH and local login setup
- Same procedure for installing ZFS
...
#### Setup backup account on the main server
...
On the backup server, create a new SSH key for the syncoid user:

```sh
ssh-keygen -t ed25519 -f /root/.ssh/id_syncoid
```
...
```sh
ssh -i /root/.ssh/id_syncoid syncoid@melchior zfs list
```

If you want to further secure the connection, you can limit syncoid to only run specific commands.
Edit `/home/syncoid/.ssh/authorized_keys` and prepend the ssh-ed25519 key with the following:

```text
command="syncoid --receive-only",no-agent-forwarding,no-port-forwarding,no-pty,no-user-rc ssh-ed25519 ...
```

#### Install Sanoid and Syncoid on the backup server

Follow the same procedure as the main server, except for the retention policy:
...
Run the replication process manually:
...
Because the main server and backup server are not on the same network, we need to tunnel the SSH connection.
...
And these should now work remotely, even if the main server is not on the same network as the backup server:
...
Let's log with commands are being executed by syncoid over SSH:
...
Run syncoid again and check the log file:
